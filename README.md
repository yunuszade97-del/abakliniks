...
Порядок показа:
1. Список детей → открыть карточку Артёма
2. Перейти в матрицу срезов — показать динамику по прошлым замерам
3. Начать новый срез — пройти несколько пунктов области A
4. У Артёма в последнем срезе A1 = 3: если поставить 2 (снижение балла), приложение потребует указать причину — показать это как встроенный контроль качества
5. Сделать паузу и вернуться — срез сохраняется автоматически, прогресс не теряется
6. Вернуться в список детей и открыть «Панель руководителя»

Если после показа нужно прогнать сценарий заново — на экране списка детей внизу есть кнопка «Сбросить демо-данные» (два нажатия подряд).
